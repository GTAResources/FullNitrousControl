...
void DrawNitrousIndicator() {
        ...
        && playa->m_pVehicle->m_nNitroBoosts > 0)
    {
        ...
        if (FNC_DRAW_IV_HUD) {
            ...
            CFont::SetColor(CRGBA(150, 30, 30, 255));
            CFont::SetOutlinePosition(1);
            CFont::SetDropColor(CRGBA(0, 0, 0, 200));
            CFont::SetScale(SCREEN_MULTIPLIER(FNC_DRAW_IVHUD_NITROUS_TEXT_WIDTH),
                SCREEN_MULTIPLIER(FNC_DRAW_IVHUD_NITROUS_TEXT_HEIGHT));
            CFont::SetCentreSize(SCREEN_WIDTH);
            CFont::PrintString(SCREEN_COORD_LEFT(FNC_DRAW_IVHUD_NITROUS_TEXT_POS_X),
                SCREEN_COORD_BOTTOM(FNC_DRAW_IVHUD_NITROUS_TEXT_POS_Y), text);
            DrawProgressBar(SCREEN_COORD_LEFT(FNC_DRAW_IVHUD_NITROUS_BAR_POS_X),
                SCREEN_COORD_BOTTOM(FNC_DRAW_IVHUD_NITROUS_BAR_POS_Y),
                SCREEN_COORD(FNC_DRAW_IVHUD_NITROUS_BAR_WIDTH), SCREEN_COORD(FNC_DRAW_IVHUD_NITROUS_BAR_HEIGHT), SCREEN_COORD(FNC_DRAW_IVHUD_NITROUS_BAR_BORDER), SCREEN_COORD(FNC_DRAW_IVHUD_NITROUS_BAR_BORDER), CRGBA(150, 30, 30, 255), CRGBA(0, 0, 0, 150), true, CRGBA(255, 255, 255, 55), progress, 0);

        }
        else if (FNC_DRAW_V_HUD) {
            CFont::SetOutlinePosition(1);
